...
def GetCountry(countriesURL):
    ...
    for tr in table:
        ...
        writer = writeCSV.WriteToCSV(fileName='countriesJSON.csv')
        writer.WriteJSON(json.dumps(temp_dict, indent=4))
        print(temp_dict)
        cnt +=1
    page.close()
    end = datetime.now()
...
def GetCountriesAsCSVFormat(countriesURL):
    start = datetime.now()
    page = requests.get(countriesURL)
    soup = bs(page.content, 'html.parser')
    table = soup.find('table', class_='table table-hover table-striped main-table').tbody.findAll('tr')
    ...
    for tr in table:
        ...
        temp_dict["cities"] = getCities(countriesURL[:-1] + temp_dict["href"])
        data.append(data)
        print(' one line added to data')
    page.close()
    # writer = writeCSV.WriteToCSV(fileName='CountriesCSVFormat.csv')
    with('CountriesCSVFormat.csv', 'w') as csvFile:
        fieldsName = [x for x in data[0].keys()]
        wr = csv.DictWriter(csvFile,fieldsName= fieldsName)
        wr.writeheader()
        for item in data:
            wr.writerow(item)
            print('one line added to CSV')
...
GetCountriesAsCSVFormat(countriesURL)
